...
7. Không thêm icon vào báo cáo vì trông nó thiếu chuyên nghiệp
...
Thứ tự: `HW04_Main_Report.md` → `BUG_REPORT.md` → `AI_Audit_Report.md` (từ AI_Log.md) → `AI_Critique.md` (200–300 từ, đếm từ và in ra số từ) → `README.md` (self-assessment + test summary, có link video demo) → `md-to-pdf.mjs` xuất PDF.
...
5. Quay video **2-trong-1** (demo automation + demo Agent Skill): ≥5 phút, thuyết minh **tiếng Việt bằng giọng thật**, có face-cam **hoặc** terminal `whoami` + `hostname`; nêu ≥1 fix mình đã sửa script AI. Upload YouTube **unlisted** → https://youtu.be/P_8rnOMATfw
...
- Khi đủ ~10 entry hoặc kết thúc 1 feature: đề nghị tổng hợp `AI_Audit_Report.md` (bảng: STT | Tool | Thời điểm | Mục đích | Prompt (rút gọn) | Output | Human action) + phần khai báo bắt buộc: _"I use AI tools for the following tasks"_.
